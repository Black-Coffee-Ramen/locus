...
DIRECTION_NAMES = {
    (-1,  0): "west (down the corridor)",
    ( 1,  0): "east (back toward elevator)",
    ( 0, -1): "south (into the rooms)",
    ( 0,  1): "north",
}

def path_to_instructions(path: List[Tuple]) -> List[str]:
    """
    ...
    for i in range(1, len(path)):
        ...
        direction = (dx, dy)
        direction_name = DIRECTION_NAMES.get(direction, f"direction ({dx},{dy})")
        ...
        if direction == current_dir:
            ...
            if current_dir is not None:
                instructions.append(
                    f"Go {DIRECTION_NAMES.get(current_dir, str(current_dir))} for {step_count} step(s)"
                )
...
class Navigator:
    ...
    def _navigate(self, start_coord: Tuple, end_room: str) -> Dict:
        ...
        return {
            ...
            "goal": goal_coord,
            "path": [list(p) for p in path],        # list of [x, y] for JSON
